# Claude Code Project Instructions — encoding-rs-kotlin

## Project Overview

This is **encoding-rs-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`encoding_rs`](https://crates.io/crates/encoding_rs).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/encoding_rs/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/encoding_rs>
